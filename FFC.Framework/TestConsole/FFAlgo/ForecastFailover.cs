...
namespace TestConsole.FFAlgo
{
    public class ForecastFailover
    {
        ...
        public string ForecastFailoverAlgorithm()
        {
            ...
            foreach (var item in db.Branches)
            {
                ...
                {
                    branchItemData = new BranchItemData() { Branch = item, ExpectedBalance = -55 };
                }
                else if (item.BranchID == 5)
                {
                    branchItemData = new BranchItemData() { Branch = item, ExpectedBalance = -21 };
                }
                else
                {
                    throw new ArgumentNullException();
